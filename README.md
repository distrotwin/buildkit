...
```
阶段一 build   版本 × 架构 × 档位，一个 job 只建一个镜像，产物 docker save 走制品
      ↓  全部完成后解锁
阶段二 test    逐个装载、真正启动、跑完整检查集 → JSON 结果 → 汇总成报告
```

**一个 job 一个镜像**，不要一个 job 串三档。三档串行会把同一批下载做三遍，而且一档失败会连带丢掉同 job 已经建好的其它档。

**selfhost 的 debootstrap 不必跨 job 共用。** 它实测只占约 4 分钟；三个档位本来并行，共用一份 stage 省的是 CPU 而不是墙钟，代价却是多一个阶段、多一次几百 MB 的制品往返和一层依赖。曾经据一个错误的测量（把「阶段 1 + micro 档阶段 3」的合计读成阶段 1 的耗时，21 分钟）加过这一层，得不偿失。
...
```sh
XFAIL="elf_broken"          # 全部档位
XFAIL_MICRO="apt_roundtrip" # 仅 micro 档
```

判据是**「这个现象在真机上也一样吗」**。银河麒麟 V4 的 gnupg 会装一个链接 libldap 的 `gpgkeys_ldap`，而本项目不装 Recommends，`ldd` 必报缺库——真实 V4 装机同样如此，镜像是忠实的，这条该进 XFAIL。反之，keyring 没拷进镜像导致 `apt-get update` 失败是真缺陷，**把真缺陷写进 XFAIL 等于把门禁关掉**。

列进 XFAIL 却通过的项会被报成 **xpass**，不判失败但必须出现在报告里：某天上游修好了、或者构建变了，那条豁免会永远挂着并掩盖真正的回归。看到 xpass 就回来把它删掉。

xfail 计入检查总数。不计的话「加豁免」就等于「让基线下降」，豁免越多、静默跳过越难被基线抓住。

## 汇总层的两道守卫

`render-report.py` 除了渲染表格，还承担两件事：

- **`--expected N`**：应当收到 N 份结果。某个 job 在跑到检查之前就挂掉（拉不到制品、装载失败、冒烟不过）根本不会产出结果 JSON，检查总数会「合法地」变低而基线放过它，结果数对不上则一定抓得住。
- **`--baseline N`**：全部结果的检查总数下限，防检查被静默跳过。这道防线原先在 `verify.sh` 里，拆成单镜像 job 之后对每个子集都失去意义，只能上移到能看见全局的这一层。

一个结果都没收到时单独判失败——否则空报告会被读成全部通过。

...
**卡死判定用 `--speed-limit` / `--speed-time`，不要用 `--max-time`。**（这条栽过两次：第二次是把它原样抄进了新写的 `fetch-squashfs.py`。**规范里写着的东西，在换一个场景重新实现时不会自动生效** —— 新写取数代码前，先回来读一遍这一节。） 这些归档站的故障形态是「返回 200、Content-Length 正确、body 一个字节都不来」，而 `--max-time` 区分不了「慢但在进」和「卡死不动」：收紧会砍掉本来能成的大文件，放宽会让一个卡死连接白等几分钟。
